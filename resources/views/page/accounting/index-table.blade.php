...
<table class="display table table-bordered table-hover accounting-table">
    ...
    <tr>
        ...
        <th colspan="4" class="text-center">Hệ số phụ cấp</th>
    ...
    @if ( $hasPayroll )
        @foreach($employees as $index => $employee)
            ...
            <tr>
                ...
                <td class="text-center sticky-col sticky-col-5">{{ $employee->salary_basic ? number_format($employee->salary_basic) : 0 }}</td>
                ...
                @php
                    $totalAllowance = $allowances->groupBy('type')->sum(function($group) use ($employee, $payroll) {
                        ...
                        return $rate * ($employee->salary_basic ?? 0);
                    });
    ...
    @else
        <tr>
            <td colspan="{{ 5 + 3 + 3 + $deductions->count() + 7 }}" class="text-left text-muted py-3">
